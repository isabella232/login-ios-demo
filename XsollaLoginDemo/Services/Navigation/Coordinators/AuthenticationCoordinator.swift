                                 ...
                                 AuthenticationCoordinatorProtocol
{
    ...
    override func start()
    {
        ...
        { [weak self] viewController, sender, formData in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - loginRequestHandler" }
        ...
        { [weak self] _, _ in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - authenticationOptionsRequestHandler" }
            self?.authenticationOptionsRequestHandler()
        }

        viewController.signupRequestHandler =
        { [weak self] viewController, sender, formData in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - signupRequestHandler \(formData)" }
            self?.signupRequestHandler(formData: formData, forVC: viewController, sender: sender)
        }

        viewController.privacyPolicyRequestHandler =
        { [weak self] in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - privacyPolicyRequestHandler" }
            self?.privacyPolicyRequestHandler()
        }

        viewController.socialNetworkLoginRequestHandler =
        { [weak self] vc, network in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - socialNetworkLoginRequestHandler \(network)" }
            self?.loginSocialNetworkRequestHandler(network: network, forVC: vc)
        }

        viewController.moreSocialNetworksRequestHandler =
        { [weak self] vc in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - moreSocialNetworksRequestHandler" }
            self?.moreSocialNetworksRequestHandler(forVC: vc)
        }

        viewController.passwordRecoveryRequestHandler =
        { [weak self] in
            logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - passwordRecoveryRequestHandler" }
    ...
    {
        logger.info(.ui, domain: .loginDemoApp) { "Auth coordinator - socialNetworkLoginRequestHandler \(network.rawValue)" }
