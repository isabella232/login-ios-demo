...
class LoginVC: BaseViewController, LoginVCProtocol
{
    ...
    {
        logger.event(.common, domain: .loginDemoApp) { "Login with \(network) pressed" }
        socialNetworkLoginRequestHandler?(self, network)
    }
    
    private func performPrivacyPolicyRequest()
    {
        logger.event(.common, domain: .loginDemoApp) { "Privacy policy pressed" }
        privacyPolicyRequestHandler?()
    }
    
    private func performLogin()
    {
        logger.event(.common, domain: .loginDemoApp) { "Login pressed" }
    ...
    {
        logger.event(.common, domain: .loginDemoApp) { "Show authentication options pressed" }
        authenticationOptionsRequestHandler?(self, authenticationOptionsButton)
    }
    
    private func performPasswordRecovery()
    {
        logger.event(.common, domain: .loginDemoApp) { "Password recovery pressed" }
        passwordRecoveryRequestHandler?()
    }
    
    private func performShowMoreSocialNetworks()
    {
        logger.event(.common, domain: .loginDemoApp) { "Show more social networks pressed" }
